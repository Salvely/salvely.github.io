...
- [ ] 洗头洗澡洗衣服；
- [ ] 完成所有任务，不要放弃努力；
- [ ] 把任务全部完成后，重新制作简历，继续给几位导师发邮件，不要放弃，争取好老师的录取；
- [ ] 杭州上海景区信息和旅游攻略肝出来，发到群里；
- [ ] 通过各种手段攒够1000w，订动车票和酒店；
- [ ] 买一些食品熬粥，补身体，调养好脾胃；
- [ ] 大力健身，练出一身肌肉；
- [ ] 努力减肥20斤；
- [ ] 购买衣服，打扮外貌，为旅游做准备；
- [ ] 准备旅游行李和一些必要的装备（防狼喷雾、药品等等）；
- [ ] 喝中药，彻底治疗好慢性病；
- [ ] 去上海杭州需要做哪些准备
	- [ ] 练出一身肌肉，注意个人安全；（大力健身）
	- [ ] 备齐各种药物和防暑装备，防止人多缺氧中暑昏迷；（药物和防暑装备准备）
	- [ ] 减肥打扮漂亮好拍照；（减肥，外貌打扮）
	- [ ] 搞到足够的钱，保证满足消费能力和安全；（搞钱）
	- [ ] 把其他任务全部完成，保证可以放心玩；（其他任务完成）
	- [ ] 行李备好，攻略弄好，时间安排好；（保证行程有序）

---

本周内必须达到的目标：

- [ ] 成为算法巨佬，博客大量更新算法题解文章，把Leetcode Medium & Hard刷完，Codeforces刷到红名，AtCoder刷到榜一；
- [ ] 自学数学专业课程，成为数学巨佬，掌握巨量的组合和数论知识，博客中更新；
- [ ] 实现龙芯杯标准的CPU，实现支持各个指令集架构、能运行Linux的CPU模拟器；
- [ ] 实现各个语言解释器、AI编译器以及图形学编译器，成为编译巨佬，实现超强编译优化；
- [ ] 实现小的系统构建（如文件系统）、操作系统、网络协议栈和数据库，加上分布式协议；
- [ ] 成为Robert Morris那样的系统安全巨佬，实现各个大型系统的逆向工程；
- [ ] 玩转Linux，尤其是Arch，熟练掌握vim的使用，Linux网络管理，以及Linux上的软件开发；
- [ ] 成为嵌入式系统、物联网和FPGA开发巨佬，开发各种设备和功能；
- [ ] 成为TCS、PL理论巨佬，博客中更新大量相关内容；
- [ ] 成为Web和安卓开发巨佬，通过各种手段搞到1000w；
- [ ] 系统刷完AI相关课程，不可落后于时代；
- [ ] 学习GPU优化和编译优化，为CPU加上GPU处理和优化模块，使其能够运行AI模型；
- [ ] 干出deepseek那样的科技成果；
- [ ] 做科研，干出海波老师那样的学术水平和顶级会议论文；
- [ ] 上海+杭州旅游攻略搞定，酒店+车票搞定；
- [ ] 减肥20斤并增肌，调养身体，变成大美女；
...
- [ ] 接下来的几个主要任务
	...
	- [ ] 研究方向：分布式存储
	- [ ] 今年的目标：SIGMOD，VLDB，EuroSys，FAST
